...
"""
Reimplementation of waitForSGEQJobs.pl, adds support for relaunching failed jobs
"""
...
# Usage: waitForSGEQJobs.py <verbose [1 or 0]> <delay in seconds in range 10-600>
# [job IDs] [job scripts]
#
#
...
#
# If any of the jobs go into error state, an error is printed to stderr and the program
# waits for the non-error
...
print(ANTSPATH)
reschedule_job = "qsub -cwd -S /bin/bash -N antsBuildTemplate_rerun -v \
ANTSPATH=%s -r yes %s | awk '{print $3}'"
...
if jobID_Pos < 0 or statePos < 0:
    print(
        "Cannot find job-ID and state field in qstat output, cannot monitor jobs\n")
    exit(1)
...
while jobsIncomplete > 0:
    ...
    print("\n\n(%s) Still waiting for %d jobs" %
          (datetime.datetime.now().ctime(), jobsIncomplete))
    if verbose:
    ...
    for j in possibly_completed_jobs:
        ...
        while status is None:
            try:
                failed = int(subprocess.check_output(
                    'qacct  -j %d | grep failed | tail -n 1' % j, shell=True).strip().split()[1])
                if failed == 0:
                    status = int(subprocess.check_output(
                    'qacct  -j %d | grep exit_status | tail -n 1' % j, shell=True).strip().split()[1])
        ...
        if status == 0:
            ...
            new_job_id = int(
                subprocess.check_output(reschedule_job % (ANTSPATH, script), shell=True))
            job_scripts[new_job_id] = script
            print("new job %d" % new_job_id)
